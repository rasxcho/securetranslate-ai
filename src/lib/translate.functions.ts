...
const TranslateInput = z.object({
  ...
  mode: z.enum(["simple", "pro", "confidential"]),
  // When true, the server MUST NOT persist source or translation.
  ephemeral: z.boolean().optional(),
});

const PROMPTS: Record<"simple" | "pro" | "confidential", string> = {
  simple:
  ...
  pro: "You are a professional English-to-German translator specializing in business, legal, and technical writing. Translate the user's text into formal, polished, native-quality German. Preserve meaning, improve grammar and style, and use formal 'Sie' where appropriate. Return ONLY the German translation with no commentary, no quotes, no prefixes.",
  confidential:
    "You are a confidential English-to-German translator. The user's text may contain placeholder tokens shaped like ⟦LABEL_1⟧ (for example ⟦EMAIL_1⟧, ⟦PHONE_2⟧, ⟦PERSON_1⟧). Treat every such token as an opaque identifier: copy it into the output EXACTLY as-is, letter-for-letter, without translating, splitting, quoting, or altering casing. Translate all surrounding text into precise, formal German. Return ONLY the German translation with no commentary.",
};
...
export const translateText = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Confidential / ephemeral requests are never written to history.
    const shouldPersist = !data.ephemeral && data.mode !== "confidential";
    if (shouldPersist) {
    ...
    return { translated, persisted: shouldPersist };
  });
